...
class HybridRetriever:
    ...
    def index_documents(self, chunks: List[DocumentChunk]) -> int:
        ...
        if len(ids) != len(set(ids)):
            ...
            raise ValueError(
                f"Expected IDs to be unique, found {len(ids)-len(set(ids))} duplicated IDs: {', '.join(duplicates[:3])}."
            )
    ...
    def _build_bm25_index(self, chunks: List[DocumentChunk]) -> None:
        ...
        tokenized_corpus = [self._tokenize_for_bm25(chunk.content) for chunk in chunks]
    ...
    def retrieve(self, query: str, session_id: str = "", top_k_semantic: int = None, top_k_bm25: int = None) -> List[RetrievedPassage]:
        ...
        for p in semantic_results:
            if p.chunk_id not in seen:
                p.score = p.score * semantic_weight
                p.retrieval_method = "hybrid"
                seen.add(p.chunk_id)
                merged.append(p)

        max_bm25_score = max((p.score for p in bm25_results), default=1.0)
        for p in bm25_results:
            norm = (p.score / max_bm25_score) if max_bm25_score > 0 else 0.0
            if p.chunk_id in seen:
                for existing in merged:
                    if existing.chunk_id == p.chunk_id:
                        existing.score += norm * bm25_weight
                        break
            else:
                p.score = norm * bm25_weight
                p.retrieval_method = "hybrid"
                seen.add(p.chunk_id)
                merged.append(p)
        ...
        filtered = []
        for p in merged:
            sanitized_content, trust_level = sanitize_rag_passage(p.content, p.chunk_id, session_id)
            p.content = sanitized_content
            p.trust_level = trust_level

            # Penalize untrusted passages slightly (prefer trusted if possible)
            if p.trust_level == "untrusted":
                p.score *= 0.6
        ...
        agent_logger.log_retrieval(
            ...
            passages=[p.to_citation_dict() for p in filtered[:5]]
        )
