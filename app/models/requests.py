...
class TransactionContext(BaseModel):
    ...
    currency: str = Field(default="EUR", description="Currency code (EUR, USD, etc.)", min_length=3, max_length=3)
    merchant: Optional[str] = Field(default=None, description="Merchant name / beneficiary")
    channel: str = Field(default="autre", description="Payment channel: online, terminal, virement, prelevement, cheque, autre")
    ...
    def validate_channel(cls, v: str) -> str:
        v = (v or "").lower().strip()
        valid = {"online", "terminal", "virement", "prelevement", "cheque", "autre"}
        return v if v in valid else "autre"
